...
class Schedule extends Model
{
    ...
    /**
     * Check that there are no conflicting schedules
     *
     * @param Schedule $schedule
     * @return bool
     */
    public static function timeslotConflict(Schedule $schedule)
    {
        return Schedule::at($schedule->time)
                       ->days($schedule->days)
                       ->when($schedule->exists, function($query) use ($schedule) {
                           $query->exclude([$schedule]);
                       })
                       ->exists();
    }
}
